...
class BadgeSlidingOverlay extends HooksOverlay {
  ...
  create(store: Store<any>, target: HTMLElement, badge: HTMLElement, ctx: Context, globalCtx: Context[] = []) {
    ...
    overlay.addEventListener('mouseenter', () => {
      overlay.style.width = `${ parseInt(overlay.style.width) + (2 + (overlay.textContent?.length || 0) + parseInt(badgeStyle.fontSize) * 3 + parseFloat(badgeStyle.marginRight) + parseFloat(badgeStyle.marginLeft)) }px`;
